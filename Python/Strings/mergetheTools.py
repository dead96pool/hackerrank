...
There are three substrings of length 3 to consider: 'AAA', 'BCA' and 'DDE'. The first substring is all 'A' characters, so u_1 = 'A'. 
The second substring has all distinct characters, so u_2 = 'BCA'. The third substring has 2 different characters, so u_3 = 'DE'. 
...
Explanation
Split s into n/k = 3 equal parts of length k = 3. Convert each t_i to u_i by removing any subsequent occurrences of 
